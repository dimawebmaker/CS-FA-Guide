		...
		if (!arg) {
			...
			switch (arg) {
				...
				case "status":
					...
					mod.command.message("Anchor mode: " + SendToStream);
					mod.command.message("Landing area: " + whichzone);
					mod.command.message("Copy map: " + whichmode);
					mod.command.message("Regional location: " + whichboss);
					mod.command.message("bossID: "   + boss_GameID);
					mod.command.message("partyMembers: " + partyMembers.length);
					break;
				default :
					mod.command.message("Invalid Parameter");
					break;
			}
		}
	});
